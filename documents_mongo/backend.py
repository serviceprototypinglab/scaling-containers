...
def get_connection(user, option, replica):
    ...
    try:
        ...
        # conn_cluster = get_database_host() + ':' + str(get_database_port())
        host1 = get_database_host() + str(replica) + ".arkis"
        conn = pymongo.MongoClient(host=host1, port=get_database_port())
    except Exception, e:
        a2 = {"mongos": e}
        print e
        conn = None
